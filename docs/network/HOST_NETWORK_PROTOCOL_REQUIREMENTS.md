...
- 设备的二进制 proto 控制链对外应按 [protocols/proto/AputureDeviceReference.proto](../../protocols/proto/AputureDeviceReference.proto) 的 `Envelope` 对接。
- 当前控制负载仍以 `RawBleControl.packets` 承载 BLE 包，不应把 typed light state 字段视为当前已启用控制面。
- 旧 `Message.proto` 仅保留兼容接收，不建议新的主机端继续发送。
...
当设备已经处于 UDP 实时流占用状态，而后续又收到来自 Matter / MQTT / BLE / Button 的状态变更时，设备会优先向最近一次 UDP 流发送端回发一个“空 AMBL 帧”（channel_count=0, payload_size=0）作为停止发送数据流的控制信号；同时也会向 UDP 组播侧广播同样的空帧，并同步把本地输出标记为 stale。

这意味着：

- 上位机/流发送端收到该帧后应停止继续发送实时流
- 设备本地会立即释放当前 UDP 流占用状态
- 之后新的非 UDP 状态会正常进入统一状态链路
...
根消息使用 `AputureDeviceReference.proto` 中的 `Envelope`。

当前应使用的消息类型:

- `MESSAGE_TYPE_DISCOVERY_REQUEST = 1001`
- `MESSAGE_TYPE_DEVICE_INFO = 1002`
- `MESSAGE_TYPE_HEARTBEAT = 1003`
- `MESSAGE_TYPE_RAW_BLE_CONTROL = 1201`

### 7.2 当前活跃 payload 语义

- 发现请求: `discovery_request`
- 设备信息回包: `device_info`
- 心跳: `heartbeat`
- 控制: `raw_ble_control.packets`

### 7.3 控制限制

- `raw_ble_control.packets` 中每个元素必须是一整包 BLE 数据
- 设备端当前会逐包桥接到 BLE 控制处理函数
- 不要假设 `StateSetRequest`、`StateReport` 已经进入当前 UDP 控制实现
